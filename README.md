...
1. Multiple User Support – Users can create new accounts or select existing ones.
2. Daily Health Log – Users can input metrics like mood, sleep hours, water intake, screen hours, fatigue, and headaches.
3. Input Validation – Ensures all user inputs are within valid ranges.
...
Variables & Data Types – Integers, floats, strings, dictionaries.
Operators & Expressions – Arithmetic and comparison operations.
Input & Type Casting– `input()`, `int()`, `float()`.
Control Flow– `if`, `elif`, `else`, `while` loops.
Functions– Both built-in and user-defined functions, with parameters and return values.
Error Handling – `try-except` blocks for input validation and API requests.
Object-Oriented Programming – Classes (`User`, `DailyLog`), methods, attributes.
File Handling & Persistence – Reading/writing JSON files to save logs.
Libraries & Modules – `requests` (API calls), `pandas` (data handling), `matplotlib` (visualization).
